...
- **Real-time Voice Chat** - Natural conversations powered by speech-to-speech (S2S) AI models:
  - **OpenAI** - gpt-realtime-2 (GPT-5-class reasoning model with configurable effort levels), gpt-realtime-1.5, gpt-realtime, gpt-realtime-mini, gpt-4o-realtime-preview, gpt-4o-mini-realtime-preview
...
3. That's it! The app supports all Realtime API models:
   - `gpt-realtime-2` (GPT-5-class reasoning model with built-in vision ✅, configurable reasoning effort: `minimal`, `low`, `medium`, `high`, `xhigh`)
   - `gpt-realtime-1.5` (GA model with built-in vision ✅, default for OpenAI Direct)
...
3. Deploy one or more of the supported models:
   - `gpt-realtime-2` (GPT-5-class reasoning model with built-in vision, configurable reasoning effort)
   - `gpt-realtime-1.5` (GA model with built-in vision)
   - `gpt-realtime` (GA model with built-in vision)
   - `gpt-realtime-mini` (Affordable GA model with built-in vision)
   - `gpt-4o-realtime-preview` (Older model - requires Groq for vision)
   - `gpt-4o-mini-realtime-preview` (Older mini model - requires Groq for vision)
...
2. Create an API key
3. Uses x.ai Voice Agent models (speech-to-speech, OpenAI Realtime API compatible)
4. Add the key to `local.properties`:
   ```properties
   XAI_API_KEY=your_xai_api_key_here
   ```
...
- **5 Distinct Voices**: `eve`, `ara`, `rex`, `sal`, `leo`
...
#### Option 4: Google Gemini Live API (Native Audio Model)
1. Go to [Google AI Studio](https://aistudio.google.com/)
2. Create an API key
3. Uses the Gemini Live API with native audio capabilities
...
- **Enables**: Alternative vision analysis provider (gpt-realtime-2, gpt-realtime-1.5, gpt-realtime, and gpt-realtime-mini have built-in vision, only required for vision analysis with gpt-4o-realtime-preview and gpt-4o-mini-realtime-preview)
...
- **OpenAI Direct** (Realtime API): Supports all six models (`gpt-realtime-2`, `gpt-realtime-1.5`, `gpt-realtime`, `gpt-realtime-mini`, `gpt-4o-realtime-preview`, `gpt-4o-mini-realtime-preview`) directly from OpenAI with 10 voices. `gpt-realtime-2` additionally exposes a **Reasoning Effort** setting (`minimal`/`low`/`medium`/`high`/`xhigh`) to trade off latency vs. reasoning depth.
- **Azure OpenAI** (Realtime API): Supports all six models with your Azure deployment, network-level isolation, and customer-managed encryption for enterprise compliance
...
| `gazeDuration` | Time looking at robot (ms) | "3000" |
| `trackAge` | Time since person detected (ms) | "10000" |
...
- Vision works automatically with `gpt-realtime-2`, `gpt-realtime-1.5`, `gpt-realtime`, and `gpt-realtime-mini` models (built-in)
